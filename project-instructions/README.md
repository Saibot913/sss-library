...
- [ ] [`src/lib/thoughtForTheDay.ts`](../src/lib/thoughtForTheDay.ts) — **data layer done, UI still to build.** `fetchThoughtForTheDay()` is implemented and returning real rows; the importer in [`tools/thought-of-the-day/`](../tools/thought-of-the-day/) is deployed and running daily. All that's left is the display:
