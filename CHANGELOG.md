...
- `fetchThoughtForTheDay()` reads the imported thought. Returns the newest row *not in the future*, using today's date in Sacramento rather than the visitor's: the emails are published on India time, so the row the importer just wrote is normally dated tomorrow, and taking the plain newest row would show it a day early. Returns null when nothing qualifies, which is the expected state on the importer's first day. Nothing renders it yet — `App.tsx` has no thought-of-the-day section.
