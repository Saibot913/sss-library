...
- Thought-for-the-day importer in `tools/thought-of-the-day/`: a Google Apps Script that parses the daily Sai Inspires group email and upserts it into a new `thought_of_the_day` table (migration `0002`), with setup instructions in its README. Email rather than scraping because all three web sources are unusable — the site renders its content with JavaScript, the archive subdomain serves a broken TLS chain, and the Google Group archive is private. Only the import side; nothing reads the table yet.
