...
// .figma/make/site.json is generated by the Figma Make platform when this
// project is running inside it (site title, favicon, analytics, etc.) — it
// doesn't exist on a plain `git clone` outside that environment, so load it
// only if present and fall back to defaults otherwise.
const projectRoot = fileURLToPath(new URL('.', import.meta.url))
const siteConfigPath = path.resolve(projectRoot, './.figma/make/site.json')
...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  const title = config.title ?? 'Sai Library | Sri Sathya Sai Baba Center of Sacramento'
  const description = config.description ?? 'Explore books and community resources at the Sri Sathya Sai Baba Center of Sacramento library.'
